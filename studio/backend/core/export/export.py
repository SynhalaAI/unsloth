...
class ExportBackend:
    ...
    def export_multi_adapter_merge(
        self,
        ...
        Args:
            adapter_paths: List of paths to adapter directories to merge
            weights: Weights for each adapter (will be normalized)
            merge_method: Merge strategy ('linear' or 'ties')
            density: For TIES method, fraction of params to keep
            save_directory: Local directory to save merged model
            format_type: Export precision/format
            compressed_method: Optional quantized export alias
            push_to_hub: Whether to push to Hugging Face Hub
            repo_id: Hub repository ID
            hf_token: Hugging Face token
        ...
        try:
            # Import the multi-adapter merge functionality
            from unsloth.multi_adapter_merge import MultiAdapterMergeConfig, merge_adapters_static

            # Validate inputs
            if len(adapter_paths) < 2:
                return False, "At least 2 adapter paths are required for multi-adapter merge", None
            if len(adapter_paths) != len(weights):
                return False, "Number of adapter paths must match number of weights", None
            ...
            # Perform the merge using the static method which handles loading internally
            logger.info(f"Merging {len(adapter_paths)} adapters with method '{merge_method}'")
            merged_model, merged_tokenizer = merge_adapters_static(
                base_model_name=self.current_checkpoint or "base_model",
            ...
            # Determine save method based on format
            _LABEL_TO_ALIAS = {
                "FP8 (compressed-tensors)": "fp8",
                "NVFP4 (compressed-tensors)": "nvfp4",
            }
            ...
            if save_directory:
                save_directory = str(resolve_export_write_dir(save_directory))
                logger.info(f"Saving merged multi-adapter model to: {save_directory}")
                ensure_dir(Path(save_directory))

                merged_token_kw = (
                    {"token": hf_token}
                    if (hf_token or is_anonymous(hf_token))
                    and _supports_kwarg(merged_model.save_pretrained_merged, "token")
                    else {}
                )
                ...
                self._write_export_metadata(save_directory)
                logger.info(f"Merged model saved successfully to {save_directory}")
                output_path = str(Path(save_directory).resolve())
            ...
            if push_to_hub:
                ...
                logger.info(f"Merged model pushed successfully to {repo_id}")

            # Cleanup
            del merged_model, merged_tokenizer
            clear_gpu_cache()

            return True, "Multi-adapter merge completed successfully", output_path

        except Exception as e:
            logger.error(f"Error merging multi-adapter: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False, f"Multi-adapter merge failed: {str(e)}", None
