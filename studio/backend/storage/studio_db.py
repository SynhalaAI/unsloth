...
_schema_ready = False
_schema_identity: tuple[str, int, int] | None = None
...
def get_connection() -> sqlite3.Connection:
    ...
    global _schema_identity, _schema_ready
    ...
    stat = db_path.stat()
    identity = (str(db_path.resolve()), stat.st_dev, stat.st_ino)
    # Notebook launchers can change UNSLOTH_STUDIO_HOME after an early caller
    # initialized storage, and users may replace/delete studio.db while this
    # process remains alive.  A process-wide boolean would then skip schema
    # creation for the newly opened database.
    if not _schema_ready or _schema_identity != identity:
        with _schema_lock:
            stat = db_path.stat()
            identity = (str(db_path.resolve()), stat.st_dev, stat.st_ino)
            if not _schema_ready or _schema_identity != identity:
